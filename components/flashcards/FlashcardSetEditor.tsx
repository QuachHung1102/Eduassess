...
export function FlashcardSetEditor({ setId, role, cards, readOnly = false }: Props) {
  ...
  return (
    ...
    <div className="grid grid-cols-1 items-start gap-6 lg:grid-cols-[340px_1fr]">
      ...
      {/* Left: form panel (sticky on desktop only) */}
      <div className="lg:sticky lg:top-6 rounded-2xl" style={{ border: "1px solid var(--border-soft)", background: "var(--surface)" }}>
        <div className="px-5 py-4" style={{ borderBottom: "1px solid var(--border-soft)" }}>
          <h2 className="text-sm font-semibold" style={{ color: "var(--foreground)" }}>Thêm thẻ mới</h2>
          <p className="mt-1 text-xs leading-5" style={{ color: "color-mix(in srgb, var(--foreground) 58%, transparent)" }}>
            {readOnly
        ...
        <div className="px-5 py-4">
          ...
          {readOnly ? (
            <div className="rounded-xl border border-dashed px-4 py-6 text-center text-sm" style={{ borderColor: "var(--border-soft)", background: "var(--surface-muted)", color: "color-mix(in srgb, var(--foreground) 55%, transparent)" }}>
              Muốn tạo bộ thẻ riêng, hãy dùng nút <strong>Tạo bộ flashcard</strong>.
            </div>
          ) : (
            <div className="space-y-4">
              <div>
                <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide" style={{ color: "color-mix(in srgb, var(--foreground) 55%, transparent)" }}>
                  Caption mặt sau
                </label>
                <textarea
                  value={draftCaption}
                  ...
                  style={{ border: "1px solid var(--border-soft)", background: "var(--surface-muted)", color: "var(--foreground)" }}
                />
              </div>
              ...
              >
                {({ open }) => (
                  <button
                    ...
                    onClick={() => open()}
                    disabled={isPending}
                      className="w-full rounded-xl px-4 py-2.5 text-sm font-medium transition-colors disabled:cursor-not-allowed disabled:opacity-60"
              ...
              <button
                ...
                className="w-full rounded-xl px-4 py-2.5 text-sm font-semibold text-white transition-colors disabled:cursor-not-allowed disabled:opacity-50"
      ...
      <div className="rounded-2xl" style={{ border: "1px solid var(--border-soft)", background: "var(--surface)" }}>
        <div className="flex items-center gap-3 px-5 py-4" style={{ borderBottom: "1px solid var(--border-soft)" }}>
          <h2 className="text-sm font-semibold" style={{ color: "var(--foreground)" }}>Danh sách thẻ</h2>
          <span className="rounded-full px-2.5 py-0.5 text-xs font-semibold" style={{ background: "var(--surface-muted)", color: "color-mix(in srgb, var(--foreground) 68%, transparent)" }}>
        ...
        <div className="p-5">
          {localCards.length === 0 ? (
            <div className="rounded-xl border border-dashed px-6 py-16 text-center" style={{ borderColor: "var(--border-soft)" }}>
              <p className="text-sm font-medium" style={{ color: "color-mix(in srgb, var(--foreground) 45%, transparent)" }}>Chưa có thẻ nào</p>
              <p className="mt-1 text-xs" style={{ color: "color-mix(in srgb, var(--foreground) 40%, transparent)" }}>Tải ảnh đầu tiên ở bảng bên trái để bắt đầu.</p>
            </div>
          ) : (
                  ...
                  {!readOnly && (
                    <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button
                        ...
                        disabled={isPending}
                        className="flex h-7 w-7 items-center justify-center rounded-full bg-white/90 shadow-sm backdrop-blur-sm hover:bg-white hover:text-blue-600 transition-colors disabled:opacity-50"
                      ...
                      <button
                        ...
                        disabled={isPending}
                        className="flex h-7 w-7 items-center justify-center rounded-full bg-white/90 shadow-sm backdrop-blur-sm hover:bg-white hover:text-rose-600 transition-colors disabled:opacity-50"
                        style={{ color: "var(--foreground)" }}
                      >
          ...
          >
            ...
            >
              ...
              <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: "1px solid var(--border-soft)" }}>
                <div>
                  <h3 className="text-sm font-semibold" style={{ color: "var(--foreground)" }}>Chỉnh sửa thẻ #{card.order}</h3>
                  <p className="mt-0.5 text-xs" style={{ color: "color-mix(in srgb, var(--foreground) 58%, transparent)" }}>Chỉnh caption cho mặt sau của thẻ</p>
                </div>
                <button
                  type="button"
                  onClick={() => setEditingCard(null)}
              ...
              <div className="px-5 py-4 space-y-4">
                {/* Image preview */}
                <div className="overflow-hidden rounded-xl" style={{ border: "1px solid var(--border-soft)" }}>
                  <CldImage
                    src={card.imageUrl}
                    alt={`Flashcard ${card.order}`}
                    width={600}
              ...
              {/* Modal footer */}
              <div className="flex gap-3 px-5 py-4" style={{ borderTop: "1px solid var(--border-soft)" }}>
                <button
                  type="button"
                  onClick={() => setEditingCard(null)}
                  disabled={isPending}
