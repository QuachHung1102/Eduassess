...
export default async function StaffRoomsPage() {
  ...
  return (
    <div className="flex flex-col h-full gap-4 sm:gap-6">
        ...
        actions={
          <>
            <Link
              ...
              className="flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg text-foreground border border-soft transition-colors hover:bg-foreground/5"
            >
            ...
            <Link href="/booking" className="clay-btn bg-primary flex items-center gap-2 px-4 py-2 text-sm font-medium text-white">
      ...
      {rooms.length === 0 ? (
        <div className="flex flex-col items-center justify-center flex-1 gap-3 text-foreground/60">
...
function RoomCard({ room }: { room: RoomItem }) {
  ...
  return (
    <div className={`clay-card flex flex-col overflow-hidden p-0 ${room.isActive ? "" : "opacity-60"}`}>
      {/* Card header */}
      <div className="px-4 py-3 flex items-start justify-between gap-2 border-b border-soft">
        <div className="min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <h2 className="font-semibold truncate text-foreground">{room.name}</h2>
            {busyNow && (
          ...
          {room.description && (
            <p className="text-xs mt-0.5 truncate text-foreground/60">{room.description}</p>
          )}
        </div>
      ...
      {/* Meta */}
      <div className="px-4 py-3 flex items-center gap-4 text-xs text-foreground/60 border-b border-soft">
      ...
      <div className="flex-1 px-4 py-3">
        {room.bookings.length === 0 ? (
          <p className="text-xs text-center py-2 text-foreground/60">
            {room.isActive ? "Trống cả ngày hôm nay" : "Không khả dụng"}
          </p>
        ) : (
          <div className="flex flex-col gap-1.5">
            <p className="text-xs font-medium mb-1 text-foreground/60">Lịch hôm nay:</p>
            {room.bookings.map((b) => (
              <div key={b.id} className="flex items-center justify-between text-xs rounded-lg px-2.5 py-1.5 bg-primary/10">
                <span className="font-medium text-primary-dark">
                  {formatTime(b.startAt)} – {formatTime(b.endAt)}
                </span>
                <span className="truncate ml-2 max-w-30 text-foreground/60">
                  {b.reason.label}
                </span>
              </div>
            ))}
      ...
      {/* Footer action */}
      {room.isActive && (
        <div className="px-4 py-2.5 border-t border-soft">
          <Link href={`/booking?roomId=${room.id}`} className="text-xs text-primary hover:underline">
            Đặt phòng này →
          </Link>
        </div>
      )}
